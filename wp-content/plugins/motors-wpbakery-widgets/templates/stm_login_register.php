...
<div class="stm-login-register-form <?php echo esc_attr( $css_class ); ?>">
	...
	<div class="row">
		...
		<div class="col-md-4">
			...
			<?php if ( boolval( apply_filters( 'is_listing', array() ) ) && defined( 'WORDPRESS_SOCIAL_LOGIN_ABS_PATH' ) ) : ?>
				...
				<form id="page-register-form" method="post">
					...
					<div class="row form-group">
						<div class="col-md-6">
							...
							<input class="user_validated_field" type="text" name="stm_user_first_name" autocomplete="given-name" placeholder="<?php esc_attr_e( 'Enter First name', 'motors-wpbakery-widgets' ); ?>"/>
						</div>
						<div class="col-md-6">
							<h4><?php esc_html_e( 'Last Name', 'motors-wpbakery-widgets' ); ?></h4>
							<input class="user_validated_field" type="text" name="stm_user_last_name" autocomplete="family-name" placeholder="<?php esc_attr_e( 'Enter Last name', 'motors-wpbakery-widgets' ); ?>"/>
						</div>
					</div>
					...
					<div class="row form-group">
						<div class="col-md-6">
							...
							<input class="user_validated_field" type="tel" name="stm_user_phone" autocomplete="tel" placeholder="<?php esc_attr_e( 'Enter Phone', 'motors-wpbakery-widgets' ); ?>"/>
						...
						<div class="col-md-6">
							...
							<input class="user_validated_field" type="email" name="stm_user_mail" autocomplete="email" placeholder="<?php esc_attr_e( 'Enter E-mail', 'motors-wpbakery-widgets' ); ?>"/>
						</div>
					</div>
					...
					<div class="row form-group">
						<div class="col-md-6">
							...
							<input class="user_validated_field" type="text" name="stm_nickname" autocomplete="username" placeholder="<?php esc_attr_e( 'Enter Login', 'motors-wpbakery-widgets' ); ?>"/>
						</div>
						<div class="col-md-6">
							<h4><?php esc_html_e( 'Password *', 'motors-wpbakery-widgets' ); ?></h4>
							<div class="stm-show-password">
								<i class="fas fa-eye-slash"></i>
								<input class="user_validated_field" type="password" name="stm_user_password" autocomplete="new-password" placeholder="<?php esc_attr_e( 'Enter Password', 'motors-wpbakery-widgets' ); ?>"/>
							</div>
						</div>
					</div>
					<?php
					...
					<?php if ( apply_filters( 'motors_vl_get_nuxy_mod', false, 'allow_user_register_as_dealer' ) ) : ?>
						<div class="stm-register-as-dealer form-group form-checker">
							<label>
								...
								<span><?php esc_html_e( 'As Dealer', 'motors-wpbakery-widgets' ); ?></span>
							</label>
						</div>
					<?php endif; ?>
					...
					<div class="form-group form-group-submit clearfix">
						...
						if ( ! empty( $recaptcha_enabled ) && $recaptcha_enabled && ! empty( $recaptcha_public_key ) && ! empty( $recaptcha_secret_key ) ) :
							?>
							<script type="text/javascript" src="https://www.google.com/recaptcha/api.js <?php // phpcs:ignore WordPress.WP.EnqueuedResources.NonEnqueuedScript ?>
							<?php
							if ( apply_filters( 'motors_vl_get_nuxy_mod', false, 'show_term_service' ) ) {
								echo '?onload=onloadRecaptchaCallback';
							}
							?>
							">
							...
							<script>
								function onSubmitPageRegister(token) {
									var form = jQuery('#page-register-form');
									jQuery.ajax({
										type: "POST",
										url: ajaxurl,
										dataType: 'json',
										...
										success: function (data) {
											if (data.user_html) {
												jQuery('#stm_user_info').append(data.user_html);
						...
						<?php else : ?>
						<input class="heading-font" type="submit" value="<?php esc_html_e( 'Sign up now!', 'motors-wpbakery-widgets' ); ?>"/>
						<?php endif; ?>
						<span class="stm-listing-loader"><i class="stm-icon-load1"></i></span>
					</div>
