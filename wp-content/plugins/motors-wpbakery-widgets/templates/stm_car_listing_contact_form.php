	...
	if ( $user_data ) :
		...
		<script>
			jQuery(document).ready(function(){
				...
				<?php
				$policy_text    = esc_html__( 'I accept the ', 'motors-wpbakery-widgets' );
				$privacy_policy = esc_html__( 'privacy policy', 'motors-wpbakery-widgets' );
				?>
				// replace privacy policy consent label
				if(jQuery('#<?php echo esc_js( $unique_id ); ?> .consent .wpcf7-list-item-label').length > 0) {
					var consent_link = '<?php echo esc_html( $policy_text ); ?> <a href="<?php echo ( get_privacy_policy_url() ) ? esc_url( get_privacy_policy_url() ) : '#!'; ?>" target="_blank"><?php echo esc_html( $privacy_policy ); ?></a>';
