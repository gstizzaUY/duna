<?php
namespace MotorsVehiclesListing\StarterTheme\Dashboard;

class Motors_Templates_System_Status {
	...
	private static function map_wp_env() {
		$mapWPEnv = array(
			...
			'mulyisite'  => array(
				'title'     => 'WP Multisite',
				'recommend' => '',
				'system'    => ( is_multisite() ) ? '<span class="mst-icon-check"></span>On' : 'Off',
			),
			'wp_debug'   => array(
				'title'     => 'WP Debug',
				'recommend' => '',
				'system'    => ( defined( 'WP_DEBUG' ) && WP_DEBUG ) ? '<span class="mst-icon-check"></span>On' : 'Off',
			),
			'lang'       => array(
				'title'     => 'Language',
				'recommend' => '',
	...
	private static function map_server_env() {
		$mapServerEnv = array(
			...
			'ziparchive'         => array(
				'title'     => 'ZipArchive',
				'recommend' => 'enabled',
				'system'    => class_exists( 'ZipArchive' ) ? '<span class="mst-icon-check"></span>enabled' : '<span class="ss-error">ZipArchive is not installed on your server, but is required if you need to import demo content.</span>',
			),
			'wp_remote_get'      => array(
				'title'     => 'WP Remote Get',
				'recommend' => 'enabled',
	...
	private static function get_compare_wp_version() {
		...
		if ( version_compare( $new, $current, '>' ) ) {
			...
			return '<span class="mst-icon-cancel"></span><span class="plug_bold">' . $current . '</span>';
		} else {
			return '<span class="mst-icon-check"></span>' . $current;
		}
	}
	...
	private static function get_memory_limit() {
		...
		return ( version_compare( $memoryNum, '268435456', '>=' ) ) ? '<span class="mst-icon-check"></span>' . $memory : '<span class="mst-icon-cancel"></span><span class="plug_bold">' . $memory . '</span>';
	}
	...
	private static function get_php_version() {
		...
		if ( version_compare( '7.4', $php_version, '>=' ) ) {
			self::$notification = true;
			$php_version        = '<span class="mst-icon-cancel"></span><span class="plug_bold">' . $php_version . '</span>';
		} else {
			$php_version = '<span class="mst-icon-check"></span>' . $php_version;
		}
	...
	private static function get_mysql_version() {
		...
		return ( version_compare( $wpdb->db_version(), '5.6', '>=' ) ) ? '<span class="mst-icon-check"></span>' . $wpdb->db_version() : '<span class="mst-icon-cancel"></span><span class="plug_bold">' . $wpdb->db_version() . '</span>';
	}
	...
	private static function get_post_max_size() {
		...
		return ( function_exists( 'ini_get' ) && version_compare( $pms, '67108864', '>=' ) ) ? '<span class="mst-icon-check"></span>' . $pmsNum : '<span class="mst-icon-cancel"></span><span class="plug_bold">' . $pmsNum . '</span>';
	}
	...
	private static function get_php_max_input_vars_system() {
		...
		if ( $max_input_vars < self::$srMaxInputVars ) {
			self::$notification = true;
			$inputVars          = '<span class="mst-icon-cancel"></span><span class="plug_bold">' . apply_filters( 'stm_theme_esc_variable', $max_input_vars ) . '</span>';
		} else {
			$inputVars = '<span class="mst-icon-check"></span>' . $max_input_vars;
		}
	...
	private static function get_max_upload_size() {
		...
		return ( version_compare( wp_max_upload_size(), '67108864', '>=' ) ) ? '<span class="mst-icon-check"></span>' . $mus : '<span class="mst-icon-cancel"></span><span class="plug_bold">' . $mus . '</span>';
	}
	...
	private static function get_php_time_limit_system() {
		...
		if ( 300 > $time_limit && 0 !== $time_limit ) {
			self::$notification = true;
			$limit              = '<span class="mst-icon-cancel"></span><span class="plug_bold">' . $time_limit . '</span>';
		} else {
			$limit = '<span class="mst-icon-check"></span>' . $time_limit;
		}
	...
	private static function get_wp_remote_get() {
		...
		return ( ! is_wp_error( $response ) && $response['response']['code'] >= 200 && $response['response']['code'] < 300 ) ? '<span class="mst-icon-check"></span>enabled' : '<span class="ss-error">wp_remote_get() failed. Some theme features may not work. Please contact your hosting provider and make sure that https://build.envato.com/api/ is not blocked.</span>';
	}
