			...
			if ( ! empty( $post['postmeta'] ) ) {
				foreach ( $post['postmeta'] as $meta ) {
					...
					if ( $key ) {
						 ...
						 */
						if ( 'elementor-hf' === get_post_type( $post_id ) || $key === '_elementor_data' ) {
							if ( is_array( $value ) ) {
								$value_json = json_encode( $value );
								$value_json = str_replace( 'motors-plugin.stylemixthemes.com', wp_parse_url( home_url(), PHP_URL_HOST ), $value_json );
								$value = json_decode( $value_json, true );
							} else {
								$value = str_replace( 'motors-plugin.stylemixthemes.com', wp_parse_url( home_url(), PHP_URL_HOST ), $value );
							}
						}
