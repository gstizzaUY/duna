 ...
 */
// phpcs:disable
class STM_WP_Import_Base extends WP_Importer {
	...
	function process_categories() {
		$this->categories = apply_filters( 'stm_wp_import_categories', $this->categories );
	...
	function process_tags() {
		$this->tags = apply_filters( 'stm_wp_import_tags', $this->tags );
	...
	function process_terms() {
		$this->terms = apply_filters( 'stm_wp_import_terms', $this->terms );
	...
	protected function process_termmeta( $term, $term_id ) {
		 ...
		 *
		 */
		$term['termmeta'] = apply_filters( 'stm_wp_import_term_meta', $term['termmeta'], $term_id, $term );
		...
		foreach ( $term['termmeta'] as $meta ) {
			 ...
			 *
			 */
			$key = apply_filters( 'stm_import_term_meta_key', $meta['key'], $term_id, $term );
			if ( ! $key ) {
				continue;
			}
			 ...
			 *
			 */
			do_action( 'stm_import_term_meta', $term_id, $key, $value );
		}
	}
	...
	function allow_create_users() {
		return apply_filters( 'stm_import_allow_create_users', true );
	}
	...
	function allow_fetch_attachments() {
		return apply_filters( 'stm_import_allow_fetch_attachments', true );
	}
	...
	function max_attachment_size() {
		return apply_filters( 'stm_import_attachment_size_limit', 0 );
	}
