...
function stm_vehicle_import_xml() {
	...
	$stm_get_taxonomies = apply_filters( 'stm_get_taxonomies', array() );
	...
	if ( ! empty( $_FILES['import_upload'] ) ) {
		if ( 'text/xml' !== $_FILES['import_upload']['type'] ) {
			$error_type = esc_html__( 'Please, upload file with .xml format' );
		}

		if ( empty( $error_type ) ) {

			$xml_file = fopen( $_FILES['import_upload']['tmp_name'], 'r+' ); //phpcs:ignore
	...
	<div id="col-container">
		<h2 class="clear"><?php esc_html_e( 'Import from XML', 'stm_vehicles_listing' ); ?></h2>
		...
		<div class="stm_xml_file_upload form-wrap">
			...
			<form id="upload_xml_form" method="post" enctype="multipart/form-data" class="wp-upload-form"
				action="<?php echo esc_url( remove_query_arg( 'error' ) ); ?>" name="stm_import_upload">
				<label class="screen-reader-text" for="import_upload"><?php esc_html_e( 'Listing file', 'stm_vehicles_listing' ); ?></label>
			...
			<?php if ( ! empty( $error_type ) ) : ?>
				<p class="danger" style="color:red;"><?php echo esc_html( $error_type ); ?></p>
			<?php endif; ?>
			...
			<?php if ( ! empty( $lines ) ) : ?>
					...
					foreach ( $lines[0] as $key => $line ) :
						...
						<?php if ( ! is_object( $line ) ) : ?>
							<div class="item" data-key="<?php echo esc_attr( $key ); ?>">
								<span class="closer">x</span><?php echo esc_html( ucfirst( $key ) ); ?>
							</div>
						<?php endif; ?>
						<?php
						$i ++;
				...
				<form method="post" action="<?php echo esc_url( remove_query_arg( 'error' ) ); ?>" name="stm_xml_import">
					<div class="target">
						<?php foreach ( $default_post_fields as $key => $default_post_field ) : ?>
							<div class="target-unit">
								<?php echo esc_attr( $default_post_field ); ?>
								<input type="hidden" name="stm_xml_field[<?php echo esc_attr( str_replace( '-', '_', sanitize_title( $default_post_field ) ) ); ?>]" value=""/>
								<div class="empty"></div>
							</div>
						<?php endforeach; ?>
			...
			if ( ! empty( $parsed_xml ) && ! empty( $_POST['stm_xml_field'] ) ) {//phpcs:ignore
				$error = false;

				$lines = $parsed_xml;

				$user_choices = sanitize_text_field( $_POST['stm_xml_field'] ); //phpcs:ignore
				...
				if ( empty( $user_choices['post_title'] ) ) {
					$error = true;
					?>
					<p class="danger" style="color:red;"><?php esc_html_e( 'Post title is required field', 'stm_vehicles_listing' ); ?></p>
					<?php
				}
				...
				if ( empty( $user_choices['post_content'] ) ) {
					$error = true;
					?>
					<p class="danger" style="color:red;"><?php esc_html_e( 'Post content is required field', 'stm_vehicles_listing' ); ?></p>
					<?php
				}
				...
				if ( empty( $user_choices['post_excerpt'] ) ) {
					$error = true;
					?>
					<p class="danger" style="color:red;"><?php esc_html_e( 'Post excerpt is required field', 'stm_vehicles_listing' ); ?></p>
					<?php
				}
				...
				if ( ! $error ) {
					foreach ( $lines as $line ) {
						...
						if ( isset( $user_choices['post_title'] ) || '0' === $user_choices['post_title'] ) {
							$title = $line[ $user_choices['post_title'] ];
						}

						if ( isset( $user_choices['post_content'] ) || '0' === $user_choices['post_content'] ) {
							$post_content = $line[ $user_choices['post_content'] ];
						}

						if ( isset( $user_choices['post_excerpt'] ) || '0' === $user_choices['post_excerpt'] ) {
							...
							foreach ( $filtered as $key => $filter_value ) {
								if ( ! empty( $filter_value ) ) {
									if ( 'sale_price' !== $key ) {
										...
										if ( ! empty( $numeric ) && ! empty( $numeric['numeric'] ) && $numeric['numeric'] ) {
											update_post_meta( $post_to_insert_id, $key, sanitize_title( $line[ $filter_value ] ) );
										} else {
											wp_add_object_terms( $post_to_insert_id, $line[ $filter_value ], $key );
											update_post_meta( $post_to_insert_id, $key, sanitize_title( $line[ $filter_value ] ) );
										}
									} else {
										update_post_meta( $post_to_insert_id, $key, sanitize_title( $line[ $filter_value ] ) );
