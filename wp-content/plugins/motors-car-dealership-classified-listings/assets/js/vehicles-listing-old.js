(function($) {
	...
	$( document ).ready(
		function ($) {
			...
			$( document ).on(
				'click',
				'.stm_listings_settings_head td:not(.manage)',
				function(){
			...
			function stmHideUseless( elementDepended, stm_this ) {
				let compareMethod = stm_this.data( 'type' ),
					depType       = $( elementDepended ).attr( 'type' ),
					depValue;
			...
			$( '.stm_vehicles_listing_option_meta input' ).each(
				function(){
					let $stmThis = $( this );
			...
			function stm_vehicles_listing_highlight( $this ) {
				let checkVal, wrapper = $this.closest( '.stm_form_wrapper' );
			...
			$( document ).on(
				'click',
				'.stm_vehicles_listing_row_actions a[href="#delete"]',
				function(e) {
			...
			$( document ).on(
				'click',
				'.stm_vehicles_listing_row_actions a[href="#save"]',
				function() {
			...
			function stmVehiclesListingSortOptions() {
				...
				$( '.listing_categories_edit .stm_listings_settings_tr' ).each(
					function(){
						$( this ).attr( 'data-tr', number );
			...
			$( '.listing_categories_add_new a[href="#cancel"]' ).on(
				'click',
				function(){
			...
			$( '.stm_vehicles_listings_add_new_row' ).on(
				'click',
				function(){
			...
			$( document ).on(
				...
				'.stm_vehicles_listing_row_actions a[href="#add_new"]',
				function(){
					let $stmForm  = $( this ).closest( 'form' ),
						$stmtable = $( this ).closest( '.stm_vehicles_add_new' ),
						message   = $( '.listing_categories_add_new .stm_response_message' );

					// validate slug
					let new_slug_input = $( '.listing_categories_add_new .stm_form_wrapper_slug input[name="slug"]' ),
						slug           = new_slug_input.val();
					// only english letters, numbers and a hyphen is allowed
					let regexp = /^\s*([0-9a-zA-Z\-]*)\s*$/;

					if ( ! regexp.test( slug ) ) {
						new_slug_input.css( 'border-color', '#ff0000' );
						return false;
					} else {
						new_slug_input.css( 'border-color', '#aaa' );
					}
			...
			$( document ).on(
				'click',
				'.stm-has-preview-image a[data-image]',
				function(e){
					e.preventDefault();
