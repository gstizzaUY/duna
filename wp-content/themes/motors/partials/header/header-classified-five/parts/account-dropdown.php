...
if ( is_user_logged_in() ) :
	...
	if ( ! is_wp_error( $user ) && ! empty( $user->data->ID ) ) :
		...
		<div class="lOffer-account-dropdown login">
			...
			<div class="name">
				<a href="<?php echo esc_url( apply_filters( 'stm_get_author_link', $user->data->ID ) ); ?>"><?php echo wp_kses_post( apply_filters( 'stm_display_user_name', $user->ID, '', '', '' ) ); ?></a>
			</div>
			<ul class="account-list">
				<li>
					<a href="<?php echo esc_url( apply_filters( 'stm_get_author_link', '' ) ); ?>">
						<?php esc_html_e( 'My items', 'motors' ); ?> (<span><?php echo esc_attr( $my_offers ); ?></span>)
					</a>
				</li>
				<?php if ( stm_show_my_plans() ) : ?>
					<li>
						<a href="<?php echo esc_url( add_query_arg( array( 'page' => 'my-plans' ), apply_filters( 'stm_get_author_link', '' ) ) ); ?>">
							<?php esc_html_e( 'My plans', 'motors' ); ?>
						</a>
					</li>
				<?php endif; ?>
				<li class="stm-my-favourites">
					<a href="<?php echo esc_url( add_query_arg( array( 'page' => 'favourite' ), apply_filters( 'stm_get_author_link', '' ) ) ); ?>">
						<?php esc_html_e( 'Favorites', 'motors' ); ?> (<span><?php echo esc_attr( $my_fav ); ?></span>)
					</a>
				</li>
	...
	<div class="lOffer-account-dropdown stm-login-form-unregistered">
		<form method="post">
			...
			<a href="<?php echo esc_url( apply_filters( 'stm_get_author_link', 'register' ) ); ?>"
				class="stm_label"><?php esc_html_e( 'Sign Up', 'motors' ); ?></a>
