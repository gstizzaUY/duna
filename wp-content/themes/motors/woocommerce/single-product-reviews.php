		...
		<div id="review_form_wrapper">
			<div id="review_form">
				<?php
					...
					if ( $account_page_url ) {
						$must_log_in_text    = __( 'You must be <a href="%s">logged in</a> to post a review.', 'motors' );
						$placeholder_count   = preg_match_all( '/(?<!%)%(\d+\$)?s/', $must_log_in_text );
						$must_log_in_args    = array_fill( 0, max( 1, $placeholder_count ), esc_url( $account_page_url ) );
						$must_log_in_message = vsprintf( $must_log_in_text, $must_log_in_args );

						$comment_form['must_log_in'] = '<p class="must-log-in">' . wp_kses(
							$must_log_in_message,
							array(
								'a' => array(
									'href' => array(),
