...
<?php else : ?>
	<h2>
		<a class="link" href="<?php echo esc_url( $order->get_edit_order_url() ); ?>"><?php printf( __( 'Order #%s', 'motors' ), $order->get_order_number() ); ?></a> (<?php printf( '<time datetime="%s">%s</time>', $order->get_date_created()->format( 'c' ), wc_format_datetime( $order->get_date_created() ) );//phpcs:ignore ?>)
	</h2>
<?php endif; ?>
